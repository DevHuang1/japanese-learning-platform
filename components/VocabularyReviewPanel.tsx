  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="paper-card rounded-2xl p-8 text-center text-sm text-paper-ink">
            ...
            return (
              <article key={review.id} aria-labelledby={titleId} className="paper-card rounded-2xl p-5">
                ...
                <div className="grid gap-4 md:grid-cols-[1fr_auto_1fr] md:items-center">
                  <div>
                    <div className="text-xs uppercase tracking-wide text-paper-ink">Incoming</div>
                    <div className="mt-1 font-jp text-2xl text-indigo-dark">{incoming.kanji ?? incoming.kana}</div>
                    <div className="font-jp text-sm text-paper-ink">{incoming.kana}</div>
                    <div className="mt-2 text-sm text-paper-ink">{incoming.burmese_meaning}</div>
                  </div>
                  ...
                  <div>
                    <div className="text-xs uppercase tracking-wide text-paper-ink">Existing candidate</div>
                    <div className="mt-1 font-jp text-2xl text-indigo-dark">{review.candidate?.kanji ?? review.candidate?.kana ?? "Deleted candidate"}</div>
                    <div className="font-jp text-sm text-paper-ink">{review.candidate?.kana}</div>
                    <div className="mt-2 text-sm text-paper-ink">{review.candidate?.burmeseMeaning}</div>
                  </div>
                </div>

                <div className="mt-4 flex flex-wrap gap-2 text-xs text-paper-ink">
                  {reasons.map((reason) => (
                ...
                <details className="mt-4 rounded-xl border border-paper-ink/10 bg-paper-deep/25 px-4 py-3">
                  ...
                  <div className="mt-3 space-y-3 text-sm text-paper-ink">
                        ...
                        <ul className="space-y-2" aria-label="Match evidence">
                          {explanation.signals.map((signal) => (
                            <li key={signal.label} className="flex items-center justify-between gap-3">
                              ...
                              <span className="text-right text-paper-ink">{signal.value} · {Math.round(signal.weight * 100)}%</span>
                            </li>
                          ))}
                        </ul>
                ...
                <div className="mt-5 flex justify-end gap-2">
                  <button
                    ...
                    disabled={busy}
                    className="inline-flex items-center gap-2 rounded-lg border border-paper-ink/15 px-3 py-2 text-sm text-paper-ink hover:bg-paper-deep/50 disabled:opacity-50"
                  >
