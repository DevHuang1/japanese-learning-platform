...
type Page = {
  id: string; pageNumber: number; mode: string; status: string; sourceImagePath: string | null; rawText: string | null;
...
export function IngestionDashboard() {
  ...
  async function action(path: string, body?: Record<string, unknown>) {
    ...
    setBusy(false);
    if (response.ok) {
      const refreshed = await fetch(`/api/ingestion/pages/${selectedPage.id}?revision=${Date.now()}`, { cache: "no-store" });
      if (refreshed.ok) {
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5" aria-label="Ingestion quality summary">
        {metricCards.map(([label, value, Icon]) => (
          <div className="paper-card rounded-2xl p-5" key={String(label)}>
            <div className="flex items-center justify-between text-paper-ink/80"><span className="text-xs font-semibold uppercase tracking-widest">{label}</span><Icon size={18} /></div>
        ...
        <div className="paper-card rounded-2xl p-5">
          ...
          <div className="space-y-2">
            ...
            {batches.map((batch) => <button key={batch.id} onClick={() => void inspectBatch(batch.id)} className="w-full rounded-xl border p-4 text-left transition hover:border-crimson/50 hover:bg-crimson/5">
              ...
              <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-paper-ink/80"><span>{batch.totalPages} pages</span><span>{batch.ocrPages} OCR</span><span>{batch.reviewCount} reviews</span></div>
            </button>)}
          </div>
        </div>

        <div className="paper-card rounded-2xl p-5">
          <div className="mb-4 flex items-center justify-between"><div><h2 className="text-lg font-semibold text-indigo-dark">OCR attention queue</h2><p className="text-sm text-paper-ink/80">Correct uncertain text before it reaches vocabulary.</p></div><span className="rounded-full bg-amber-50 px-3 py-1 text-sm text-amber-900">{summary?.attention.pages ?? 0} pages</span></div>
          {pages.length === 0 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-paper-ink/80">Select an ingestion batch to open its correction queue.</div> : <div className="space-y-2">{pages.map((page) => <button key={page.id} onClick={() => choosePage(page)} className={`w-full rounded-xl border p-3 text-left ${selectedPage?.id === page.id ? "border-crimson bg-crimson/5" : "hover:bg-paper-deep/30"}`}><div className="flex items-center justify-between"><span className="font-medium">Page {page.pageNumber}</span><span className={`rounded-full border px-2 py-1 text-xs ${statusClass(page.status)}`}>{page.status}</span></div><p className="mt-1 text-xs text-paper-ink/80">{page.mode} · {page.ocrConfidence !== null ? `${Math.round(page.ocrConfidence * 100)}% OCR confidence` : "confidence unavailable"} · {page.candidates.length} candidates</p></button>)}</div>}
        </div>
      </section>

      {selectedPage && <section className="paper-card rounded-2xl p-5" aria-label="OCR correction workspace">
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Sparkles size={18} className="text-crimson" /><h2 className="text-lg font-semibold text-indigo-dark">OCR correction workspace · page {selectedPage.pageNumber}</h2></div><p className="mt-1 text-sm text-paper-ink/80">Revision {selectedPage.revision}. Original OCR remains in the correction history.</p></div><span className={`rounded-full border px-3 py-1 text-sm ${statusClass(selectedPage.status)}`}>{selectedPage.status}</span></div>
                <div className="grid gap-5 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="space-y-4">
            <div className="overflow-hidden rounded-xl border bg-paper-deep/30">
              {selectedPage.sourceImagePath ? (
                <figure>
                  <div className="max-h-[32rem] overflow-auto bg-slate-100 p-3">
                    <Image
                      src={`/api/ingestion/pages/${selectedPage.id}/image`}
                      alt={`Scanned source preview for page ${selectedPage.pageNumber}`}
                      width={1200}
                      height={1600}
                      unoptimized
